...
class AutoencoderKLAllegro(ModelMixin, ConfigMixin):
    ...
    @codeflash_capture(function_name='AutoencoderKLAllegro.__init__', tmp_dir_path='/tmp/codeflash_q99d7bv8/test_return_values', tests_root='/home/ubuntu/codeflash-dev/diffusers/tests', is_fto=True)
    @register_to_config
    def __init__(self, in_channels: int=3, out_channels: int=3, down_block_types: Tuple[str, ...]=('AllegroDownBlock3D', 'AllegroDownBlock3D', 'AllegroDownBlock3D', 'AllegroDownBlock3D'), up_block_types: Tuple[str, ...]=('AllegroUpBlock3D', 'AllegroUpBlock3D', 'AllegroUpBlock3D', 'AllegroUpBlock3D'), block_out_channels: Tuple[int, ...]=(128, 256, 512, 512), temporal_downsample_blocks: Tuple[bool, ...]=(True, True, False, False), temporal_upsample_blocks: Tuple[bool, ...]=(False, True, True, False), latent_channels: int=4, layers_per_block: int=2, act_fn: str='silu', norm_num_groups: int=32, temporal_compression_ratio: float=4, sample_size: int=320, scaling_factor: float=0.13, force_upcast: bool=True) -> None:
        super().__init__()
        self.encoder = AllegroEncoder3D(in_channels=in_channels, out_channels=latent_channels, down_block_types=down_block_types, temporal_downsample_blocks=temporal_downsample_blocks, block_out_channels=block_out_channels, layers_per_block=layers_per_block, act_fn=act_fn, norm_num_groups=norm_num_groups, double_z=True)
        self.decoder = AllegroDecoder3D(in_channels=latent_channels, out_channels=out_channels, up_block_types=up_block_types, temporal_upsample_blocks=temporal_upsample_blocks, block_out_channels=block_out_channels, layers_per_block=layers_per_block, norm_num_groups=norm_num_groups, act_fn=act_fn)
        ...
        self.stride = (sample_frames - self.tile_overlap_t, sample_size - self.tile_overlap_h, sample_size - self.tile_overlap_w)
    ...
    def tiled_encode(self, x: torch.Tensor) -> torch.Tensor:
        ...
        count = 0
        output_latent = x.new_zeros((output_num_frames * output_height * output_width, 2 * self.config.latent_channels, self.kernel[0] // rt, self.kernel[1] // rs, self.kernel[2] // rs))
        ...
        for i in range(output_num_frames):
            ...
            for j in range(output_height):
                ...
                for k in range(output_width):
                    ...
                    latent_mean = _prepare_for_blend((i, output_num_frames, output_overlap[0]), (j, output_height, output_overlap[1]), (k, output_width, output_overlap[2]), output_latent[i * output_height * output_width + j * output_width + k].unsqueeze(0))
    ...
    def tiled_decode(self, z: torch.Tensor) -> torch.Tensor:
        ...
        count = 0
        decoded_videos = z.new_zeros((output_num_frames * output_height * output_width, self.config.out_channels, self.kernel[0], self.kernel[1], self.kernel[2]))
        vae_batch_input = z.new_zeros((local_batch_size, num_channels, latent_kernel[0], latent_kernel[1], latent_kernel[2]))
        for i in range(output_num_frames):
            for j in range(output_height):
                for k in range(output_width):
        ...
        for i in range(output_num_frames):
            ...
            for j in range(output_height):
                ...
                for k in range(output_width):
                    w_start, w_end = (k * self.stride[2], k * self.stride[2] + self.kernel[2])
                    out_video_blend = _prepare_for_blend((i, output_num_frames, video_overlap[0]), (j, output_height, video_overlap[1]), (k, output_width, video_overlap[2]), decoded_videos[i * output_height * output_width + j * output_width + k].unsqueeze(0))
...
def _prepare_for_blend(n_param, h_param, w_param, x):
    ...
    if overlap_n > 0:
        if n > 0:
            mask = _get_blend_mask('n', overlap_n, device, reverse=False)
            # mask shape (overlap_n,1,1) will broadcast to x[:, :, 0:overlap_n, :, :]
            x[:, :, 0:overlap_n, :, :] = x[:, :, 0:overlap_n, :, :] * mask
        if n < n_max - 1:
            mask = _get_blend_mask('n', overlap_n, device, reverse=True)
            x[:, :, -overlap_n:, :, :] = x[:, :, -overlap_n:, :, :] * mask

    if overlap_h > 0:
        if h > 0:
            mask = _get_blend_mask('h', overlap_h, device, reverse=False)
            # mask shape (overlap_h,1), broadcast to x[:, :, :, 0:overlap_h, :]
            x[:, :, :, 0:overlap_h, :] = x[:, :, :, 0:overlap_h, :] * mask
        if h < h_max - 1:
            mask = _get_blend_mask('h', overlap_h, device, reverse=True)
            x[:, :, :, -overlap_h:, :] = x[:, :, :, -overlap_h:, :] * mask

    if overlap_w > 0:
        if w > 0:
            mask = _get_blend_mask('w', overlap_w, device, reverse=False)
            # mask is shape (overlap_w,)
            x[:, :, :, :, 0:overlap_w] = x[:, :, :, :, 0:overlap_w] * mask
        if w < w_max - 1:
            mask = _get_blend_mask('w', overlap_w, device, reverse=True)
            x[:, :, :, :, -overlap_w:] = x[:, :, :, :, -overlap_w:] * mask

    return x

def _get_blend_mask(dim, overlap, device, reverse=False):
    """Get (and cache) blend mask as shape (overlap, 1, 1) or (overlap, 1), as float32 on the given device."""
    if overlap <= 0:
        return None
    key = (dim, overlap, device, reverse)
    if key in _BLEND_CACHE:
        return _BLEND_CACHE[key]
    arange = torch.arange(0, overlap, device=device, dtype=torch.float32) / overlap
    if reverse:
        arange = 1.0 - arange
    if dim == 'n':
        mask = arange.view(overlap, 1, 1)
    elif dim == 'h':
        mask = arange.view(overlap, 1)
    elif dim == 'w':
        mask = arange  # already 1D, will broadcast in-place
    else:
        raise ValueError("Unknown dim")
    _BLEND_CACHE[key] = mask
    return mask
