...
class FreeInitMixin:
    ...
    def _apply_freq_filter(self, x: torch.Tensor, noise: torch.Tensor, low_pass_filter: torch.Tensor) -> torch.Tensor:
        ...
        # Compute and shift FFTs in one step each
        x_freq = fft.fftshift(fft.fftn(x, dim=dims), dim=dims)
        noise_freq = fft.fftshift(fft.fftn(noise, dim=dims), dim=dims)

        # Reuse buffers and operate in-place where safe
        high_pass_filter = torch.sub(1, low_pass_filter)
        x_freq.mul_(low_pass_filter)
        noise_freq.mul_(high_pass_filter)
        x_freq.add_(noise_freq)  # x_freq now holds freq-domain mix

        # Shift back and IFFT, then take real part
        x_freq_mixed = fft.ifftshift(x_freq, dim=dims)
