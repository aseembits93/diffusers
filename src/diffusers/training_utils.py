...
def compute_snr(noise_scheduler, timesteps):
    ...
    # Move alphas_cumprod to correct device and dtype just once.
    alphas_cumprod = alphas_cumprod.to(device=timesteps.device, dtype=torch.float32)

    # Gather the needed alphas only
    selected_alphas = alphas_cumprod[timesteps]
    # Calculate SNR directly, no need for creating separate expand/reshape steps
    # SNR = alpha / (1-alpha)
    snr = selected_alphas / (1.0 - selected_alphas)
